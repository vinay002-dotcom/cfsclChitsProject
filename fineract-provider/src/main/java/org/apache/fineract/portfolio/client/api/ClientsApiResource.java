...
public class ClientsApiResource {
    ...
    public ClientsApiResource(final PlatformSecurityContext context, final ClientReadPlatformService readPlatformService,
            ...
            final BulkImportWorkbookService bulkImportWorkbookService, final FromJsonHelper fromApiJsonHelper,
     final CreditReportsReadPlatformService cibilreadPlatformService) {
    ...
    public String karzaCall(@Parameter(hidden = true) final String apiRequestBodyAsJson) {

        ////System.out.println("Here in KARZA----");
    ...
    public String retrieveAll(@Context final UriInfo uriInfo,
            ...
            @QueryParam("paged") @Parameter(description = "paged") final Boolean paged, @QueryParam("staffId") @Parameter(description = "staffId") final Long staffId) {
		...
		final SearchParameters searchParameters = SearchParameters.forClients(sqlSearch, officeId, externalId, displayName, firstname,
                lastname, status, hierarchy, offset, limit, orderBy, sortOrder, orphansOnly, isSelfUser,staffId );
		////System.out.println(paged);
		////System.out.println(parameters.isPaged());
        if (parameters.isPaged()) {
        	////System.out.println("in paged");
        	 final Page<ClientData> clientData = this.clientReadPlatformService.retrievePagedAll(searchParameters,parameters);
        	 return this.toApiJsonSerializer.serialize(settings, clientData, ClientApiConstants.CLIENT_RESPONSE_DATA_PARAMETERS);
        }
    ...
    public String retrieveOne(@PathParam("clientId") @Parameter(description = "clientId") final Long clientId,
    	...
    	Collection<CreditReport> read = cibilreadPlatformService.getreport(clientId);
    	...
    	for(int i=0;i<CreditReportList.size();i++)
		{
			...
			{
				CreditReport objcreditreport=(CreditReport)obj;
				ScoreValue=Long.parseLong(objcreditreport.getScorevalue());  
				ScoreType = objcreditreport.getScorecomments();
				DateOfIssue = objcreditreport.getDateofissue();
				break;
			}
		}
    	
        final JsonElement requestelement = this.fromApiJsonHelper.parse(this.toApiJsonSerializer.serialize(settings, clientData, ClientApiConstants.CLIENT_RESPONSE_DATA_PARAMETERS));
        final JsonObject re = requestelement.getAsJsonObject();
        
        
        re.addProperty("scorevalue", ScoreValue);
        re.addProperty("scoreType", ScoreType);
        re.addProperty("DateOfIssue", DateOfIssue);
        String response  = re.toString();
        return response;
    }
    ...
    @Operation(summary = "Create a Client", description = "Note:\n\n"
            ...
            + "Mandatory Fields: firstname  OR fullname, officeId, active=true and activationDate OR active=false, if(address enabled) address\n\n"
