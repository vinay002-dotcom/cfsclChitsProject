...
public class ClientTransactionNotFoundException extends AbstractPlatformDomainRuleException {
    ...
    public ClientTransactionNotFoundException(final Long clientId) {
        super("error.msg.client.transaction.not.found.exception",
                "There are no Transaction with Clientid `" + clientId + "` does not exist for a Client with id `" + clientId,
                clientId);
    }

    public ClientTransactionNotFoundException(EmptyResultDataAccessException e) {
        super("error.msg.client.transaction.not.found.exception",
                "No Client Transactions were found",
                e);
    }
}
