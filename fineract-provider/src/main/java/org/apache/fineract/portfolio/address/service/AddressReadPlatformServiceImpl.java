...
public class AddressReadPlatformServiceImpl implements AddressReadPlatformService {
	...
	private static final class AddFieldsMapper implements RowMapper<AddressData> {
		public String schema() {
			return "addr.id as id,client.id as client_id,addr.houseNo as houseNo,addr.street as street,addr.address_line_1 as address_line_1,addr.address_line_2 as address_line_2,"
					...
					+ "addr.taluka_id as taluka_id, addr.district_id as district_id,"
		...
		public AddressData mapRow(final ResultSet rs, @SuppressWarnings("unused") final int rowNum)
			...
			//final String landLine = rs.getString("landLine");
			
			//final String mobileNum = rs.getString("mobileNum");
			...
			final long district_id = rs.getLong("district_id");

			final long taluka_id = rs.getLong("taluka_id");
			...
			return AddressData.instance1(addressId, houseNo,street, address_line_1, address_line_2, address_line_3,
					town_village, city, county_district, taluka_id, district_id, state_province_id, country_id, postal_code, latitude,
	...
	private static final class AddMapper implements RowMapper<AddressData> {
		public String schema() {
			return "cv2.code_value as addressType,ca.client_id as client_id,addr.id as id,ca.address_type_id as addresstyp,ca.is_active as is_active,addr.houseNo as houseNo,addr.street as street,addr.address_line_1 as address_line_1,addr.address_line_2 as address_line_2,"
					+ "addr.address_line_3 as address_line_3,addr.town_village as town_village, addr.city as city,addr.county_district as county_district,"
					+ "addr.taluka_id as taluka_id, t.code_value as taluka_name, addr.district_id as district_id, d.code_value as district_name,"
					...
					+ "addr.longitude as longitude,addr.created_by as created_by,addr.created_on as created_on,addr.updated_by as updated_by,"
					+ "addr.updated_on as updated_on, addr.taluka as taluka, addr.district as district, addr.landmark as landmark "
					...
					+ " left join  m_code_value d on addr.district_id=d.id"
					+ " left join  m_code_value t on addr.taluka_id=t.id"
		...
		public AddressData mapRow(final ResultSet rs, @SuppressWarnings("unused") final int rowNum)
			...
			final String district_name = rs.getString("district_name");

			final long taluka_id = rs.getLong("taluka_id");

			final String taluka_name = rs.getString("taluka_name");
			...
			return AddressData.instance3(addressType, client_id, addressId, address_type_id, is_active, houseNo, street,
					address_line_1, address_line_2, address_line_3, town_village, city, county_district, taluka_id, district_id,
					state_province_id, country_id, taluka_name, district_name, state_name, country_name, postal_code, latitude, longitude,
					created_by, created_on, updated_by, updated_on, taluka, district,landmark);
	...
	public AddressData retrieveTemplate() {
		...
		final List<CodeValueData> talukaOptions = new ArrayList<>(
			this.readService.retrieveCodeValuesByCode("TALUKA"));

		final List<CodeValueData> districtOptions = new ArrayList<>(
			this.readService.retrieveCodeValuesByCode("DISTRICT"));
		...
		return AddressData.template(talukaOptions, districtOptions, StateOptions, countryoptions, addressTypeOptions);
	}
}