...
public class PaymentDetailWritePlatformServiceJpaRepositoryImpl implements PaymentDetailWritePlatformService {
	...
	@Override
    public PaymentDetail createPaymentDetailJson(final JsonObject command, final Map<String, Object> changes) {
        final Long paymentTypeId = command.get(PaymentDetailConstants.paymentTypeParamName).getAsLong();
        if (paymentTypeId == null) {
            return null;
        }
        
        final PaymentType paymentType = this.paymentTyperepositoryWrapper.findOneWithNotFoundDetection(paymentTypeId);
        final PaymentDetail paymentDetail = PaymentDetail.generatePaymentDetailJson(paymentType, command, changes);
        return paymentDetail;
    ...
    @Override
    @Transactional
    public PaymentDetail createAndPersistPaymentDetailJson(final JsonObject command, final Map<String, Object> changes) {
        final PaymentDetail paymentDetail = createPaymentDetailJson(command, changes);
        if (paymentDetail != null) {
            return persistPaymentDetail(paymentDetail);
        }
        return paymentDetail;
