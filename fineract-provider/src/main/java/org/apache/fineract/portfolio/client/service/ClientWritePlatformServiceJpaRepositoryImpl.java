...
//import org.apache.fineract.portfolio.client.exception.CreditScoreUnsatisFactoryException;
...
public class ClientWritePlatformServiceJpaRepositoryImpl implements ClientWritePlatformService {
	...
	private final EntityDatatableChecksWritePlatformService entityDatatableChecksWritePlatformService;
	 ...
	 private final AddressReadPlatformService adressreadservices;
		private final ClientAddressApiResources adressapiresource;
	...
	public ClientWritePlatformServiceJpaRepositoryImpl(final PlatformSecurityContext context,
			...
			final EntityDatatableChecksWritePlatformService entityDatatableChecksWritePlatformService, final CreditReportsReadPlatformService creditreadPlatformService,
			 final AddressReadPlatformService adressreadservices,final ClientAddressApiResources adressapiresource) {
		...
		this.adressapiresource = adressapiresource;
	}
	...
	private void handleDataIntegrityIssues(final JsonCommand command, final Throwable realCause, final Exception dve) {
		...
		if (realCause.getMessage().contains("external_id")) {
			...
			String local = realCause.getMessage().substring(0, 54);
			////System.out.println(local);
			if(local.startsWith("Duplicate entry"))
			{
				if (local.contains("external_id")) {
				...
				} else if (local.contains("account_no_UNIQUE")) {
				...
				} else if (local.contains("mobile_no_UNIQUE")) {
	...
	public CommandProcessingResult createClient(final JsonCommand command) {
		...
		try {
			...
			CodeValue statusOne = null;
			final Long statusOneId = command.longValueOfParameterNamed(ClientApiConstants.statusOneIdParamName);
			if (statusOneId != null) {
				statusOne = this.codeValueRepository.findOneByCodeNameAndIdWithNotFoundDetection(ClientApiConstants.STATUSONE, statusOneId);
			}

			CodeValue statusTwo = null;
			final Long statusTwoId = command.longValueOfParameterNamed(ClientApiConstants.statusTwoIdParamName);
			if (statusTwoId != null) {
				statusTwo = this.codeValueRepository.findOneByCodeNameAndIdWithNotFoundDetection(ClientApiConstants.STATUSTWO, statusTwoId);
			}

			CodeValue introducer = null;
			final Long introducerId = command.longValueOfParameterNamed(ClientApiConstants.introducerIdParamName);
			if (introducerId != null) {
				introducer = this.codeValueRepository.findOneByCodeNameAndIdWithNotFoundDetection(ClientApiConstants.INTRODUCER, introducerId);
			}

			CodeValue sourceone = null;
			final Long sourceoneId = command.longValueOfParameterNamed(ClientApiConstants.sourceoneIdParamName);
			if (sourceoneId != null) {
				sourceone = this.codeValueRepository.findOneByCodeNameAndIdWithNotFoundDetection(ClientApiConstants.SOURCEONE, sourceoneId);
			}

			CodeValue sourcetwo = null;
			final Long sourcetwoId = command.longValueOfParameterNamed(ClientApiConstants.sourcetwoIdParamName);
			if (sourcetwoId != null) {
				sourcetwo = this.codeValueRepository.findOneByCodeNameAndIdWithNotFoundDetection(ClientApiConstants.SOURCETWO, sourcetwoId);
			}

			CodeValue purposeone = null;
			final Long purposeoneId = command.longValueOfParameterNamed(ClientApiConstants.purposeoneIdParamName);
			if (purposeoneId != null) {
				purposeone = this.codeValueRepository.findOneByCodeNameAndIdWithNotFoundDetection(ClientApiConstants.PURPOSEONE, purposeoneId);
			}

			CodeValue purposetwo = null;
			final Long purposetwoId = command.longValueOfParameterNamed(ClientApiConstants.purposetwoIdParamName);
			if (purposetwoId != null) {
				purposetwo = this.codeValueRepository.findOneByCodeNameAndIdWithNotFoundDetection(ClientApiConstants.PURPOSETWO, purposetwoId);
			}


			CodeValue fatherspouse = null;
			final Long fatherspouseId = command.longValueOfParameterNamed(ClientApiConstants.fatherspouseIdParamName);
			if (fatherspouseId != null) {
				fatherspouse = this.codeValueRepository.findOneByCodeNameAndIdWithNotFoundDetection(ClientApiConstants.FATHERSPOUSE, fatherspouseId);
			}
			...
			final Long professionId = command.longValueOfParameterNamed(ClientApiConstants.professionIdParamName);
			if (professionId != null) {
				profession = this.codeValueRepository.findOneByCodeNameAndIdWithNotFoundDetection(ClientApiConstants.PROFESSION, professionId);
			}

			CodeValue belonging = null;
			final Long belongingId = command.longValueOfParameterNamed(ClientApiConstants.belongingIdParamName);
			if (belongingId != null) {
				belonging = this.codeValueRepository.findOneByCodeNameAndIdWithNotFoundDetection(ClientApiConstants.BELONGING, belongingId);
			}

			CodeValue annual = null;
			final Long annualId = command.longValueOfParameterNamed(ClientApiConstants.annualIdParamName);
			if (annualId != null) {
				annual = this.codeValueRepository.findOneByCodeNameAndIdWithNotFoundDetection(ClientApiConstants.ANNUAL, annualId);
			}

			CodeValue land = null;
			final Long landId = command.longValueOfParameterNamed(ClientApiConstants.landIdParamName);
			if (landId != null) {
				land = this.codeValueRepository.findOneByCodeNameAndIdWithNotFoundDetection(ClientApiConstants.LAND, landId);
			}

			CodeValue house = null;
			final Long houseId = command.longValueOfParameterNamed(ClientApiConstants.houseIdParamName);
			if (houseId != null) {
				house = this.codeValueRepository.findOneByCodeNameAndIdWithNotFoundDetection(ClientApiConstants.HOUSE, houseId);
			}

			CodeValue form = null;
			final Long formId = command.longValueOfParameterNamed(ClientApiConstants.formIdParamName);
			if (formId != null) {
				form = this.codeValueRepository.findOneByCodeNameAndIdWithNotFoundDetection(ClientApiConstants.FORM, formId);
			}

			CodeValue title = null;
			final Long titleId = command.longValueOfParameterNamed(ClientApiConstants.titleIdParamName);
			if (titleId != null) {
				title = this.codeValueRepository.findOneByCodeNameAndIdWithNotFoundDetection(ClientApiConstants.TITLE, titleId);
			}
			...
			CodeValue alternateNo = null;
			final Long alternateNoId = command.longValueOfParameterNamed(ClientApiConstants.alternateNoIdParamName);
			if (alternateNoId != null) {
				alternateNo = this.codeValueRepository.findOneByCodeNameAndIdWithNotFoundDetection(ClientApiConstants.ALTNO, alternateNoId);
			}

			CodeValue idproof = null;
			final Long idproofId = command.longValueOfParameterNamed(ClientApiConstants.idproofIdParamName);
			if (idproofId != null) {
				idproof = this.codeValueRepository.findOneByCodeNameAndIdWithNotFoundDetection(ClientApiConstants.IDPROOF, idproofId);
			}

			CodeValue addrproof = null;
			final Long addrproofId = command.longValueOfParameterNamed(ClientApiConstants.addrproofIdParamName);
			if (addrproofId != null) {
				addrproof = this.codeValueRepository.findOneByCodeNameAndIdWithNotFoundDetection(ClientApiConstants.ADDRPROOF, addrproofId);
			}
			...
			final Client newClient = Client.createNew(currentUser, clientOffice, clientParentGroup, staff, savingsProductId,
					gender,statusOne, statusTwo,introducer,sourceone, sourcetwo, purposeone, purposetwo,fatherspouse,
					education, marital, profession, belonging, annual,
					land, house, form, title, religion,alternateNo, idproof, addrproof, clientType,  clientClassification, legalFormValue, command);
			...
			//            if(command.arrayOfParameterNamed("address")!=null)
			//            {
			//            	this.addressWritePlatformService.addNewClientAddress(newClient, command);
			//            }
			//            
	...
	public CommandProcessingResult updateClient(final Long clientId, final JsonCommand command) {
		...
		try {
			...
			{
				Long addressTypeId;
				Collection<AddressData> clientaddress = adressreadservices.retrieveAllClientAddress(clientForUpdate.getId());
				List<AddressData> clientaddressList = new ArrayList<AddressData>(clientaddress);
				for(int i = 0 ; i < command.arrayOfParameterNamed("address").size() ; i++)
				{
					Boolean flag = true ; 
					JsonElement ArrayofAddress = command.arrayOfParameterNamed("address").get(i);
					JsonObject Arrayofadres = ArrayofAddress.getAsJsonObject();
					addressTypeId = Arrayofadres.get("addressTypeId").getAsLong();
					
					for(int j = 0 ; j < clientaddressList.size() ; j++ )
					{
						AddressData addres = clientaddressList.get(j);
						Long adId = addres.getAddressTypeId();
						if(addressTypeId.equals(adId))
						{
							Arrayofadres.addProperty("addressId", addres.getAddressId());
							Arrayofadres.remove("addressTypeId");
							adressapiresource.updateClientAddress(clientForUpdate.getId(), Arrayofadres.toString());
							flag = false;
							break;
						}
					}
					...
					if(flag)
					{
						adressapiresource.addClientAddress(addressTypeId,clientForUpdate.getId() , Arrayofadres.toString());
					}
				}
			}
			...
			if (changes.containsKey(ClientApiConstants.fatherspouseIdParamName)) {

				final Long newValue = command.longValueOfParameterNamed(ClientApiConstants.fatherspouseIdParamName);
				CodeValue fatherspouse = null;
				if (newValue != null) {
					fatherspouse = this.codeValueRepository.findOneByCodeNameAndIdWithNotFoundDetection(ClientApiConstants.FATHERSPOUSE, newValue);
				}
				clientForUpdate.updateFatherspouse(fatherspouse);
			}
			...
			if (changes.containsKey(ClientApiConstants.professionIdParamName)) {
				...
				CodeValue profession = null;
				if (newValue != null) {
					profession = this.codeValueRepository.findOneByCodeNameAndIdWithNotFoundDetection(ClientApiConstants.PROFESSION, newValue);
				}
				clientForUpdate.updateProfession(profession);
			}
			if (changes.containsKey(ClientApiConstants.belongingIdParamName)) {

				final Long newValue = command.longValueOfParameterNamed(ClientApiConstants.belongingIdParamName);
				CodeValue belonging = null;
				if (newValue != null) {
					belonging = this.codeValueRepository.findOneByCodeNameAndIdWithNotFoundDetection(ClientApiConstants.BELONGING, newValue);
				}
				clientForUpdate.updateBelonging(belonging);
			}

			if (changes.containsKey(ClientApiConstants.annualIdParamName)) {

				final Long newValue = command.longValueOfParameterNamed(ClientApiConstants.annualIdParamName);
				CodeValue annual = null;
				if (newValue != null) {
					annual = this.codeValueRepository.findOneByCodeNameAndIdWithNotFoundDetection(ClientApiConstants.ANNUAL, newValue);
				}
				clientForUpdate.updateAnnual(annual);
			}

			if (changes.containsKey(ClientApiConstants.landIdParamName)) {

				final Long newValue = command.longValueOfParameterNamed(ClientApiConstants.landIdParamName);
				CodeValue land = null;
				if (newValue != null) {
					land = this.codeValueRepository.findOneByCodeNameAndIdWithNotFoundDetection(ClientApiConstants.EDUCATION, newValue);
				}
				clientForUpdate.updateLand(land);
			}

			if (changes.containsKey(ClientApiConstants.houseIdParamName)) {

				final Long newValue = command.longValueOfParameterNamed(ClientApiConstants.houseIdParamName);
				CodeValue house = null;
				if (newValue != null) {
					house = this.codeValueRepository.findOneByCodeNameAndIdWithNotFoundDetection(ClientApiConstants.HOUSE, newValue);
				}
				clientForUpdate.updateHouse(house);
			}

			if (changes.containsKey(ClientApiConstants.formIdParamName)) {

				final Long newValue = command.longValueOfParameterNamed(ClientApiConstants.formIdParamName);
				CodeValue form = null;
				if (newValue != null) {
					form = this.codeValueRepository.findOneByCodeNameAndIdWithNotFoundDetection(ClientApiConstants.FORM, newValue);
				}
				clientForUpdate.updateForm(form);
			}
			if (changes.containsKey(ClientApiConstants.titleIdParamName)) {
				...
				CodeValue title = null;
				if (newValue != null) {
					title = this.codeValueRepository.findOneByCodeNameAndIdWithNotFoundDetection(ClientApiConstants.TITLE, newValue);
				}
			...
			if (changes.containsKey(ClientApiConstants.alternateNoIdParamName)) {

				final Long newValue = command.longValueOfParameterNamed(ClientApiConstants.alternateNoIdParamName);
				CodeValue alternateNo = null;
				if (newValue != null) {
					alternateNo = this.codeValueRepository.findOneByCodeNameAndIdWithNotFoundDetection(ClientApiConstants.ALTNO, newValue);
				}
				clientForUpdate.updateAlternateNo(alternateNo);
			}

			if (changes.containsKey(ClientApiConstants.idproofIdParamName)) {

				final Long newValue = command.longValueOfParameterNamed(ClientApiConstants.idproofIdParamName);
				CodeValue idproof = null;
				if (newValue != null) {
					idproof = this.codeValueRepository.findOneByCodeNameAndIdWithNotFoundDetection(ClientApiConstants.IDPROOF, newValue);
				}
				clientForUpdate.updateIdproof(idproof);
			}

			if (changes.containsKey(ClientApiConstants.addrproofIdParamName)) {

				final Long newValue = command.longValueOfParameterNamed(ClientApiConstants.addrproofIdParamName);
				CodeValue addrproof = null;
				if (newValue != null) {
					addrproof = this.codeValueRepository.findOneByCodeNameAndIdWithNotFoundDetection(ClientApiConstants.ADDRPROOF, newValue);
				}
				clientForUpdate.updateAddrproof(addrproof);
			}

			if(command.arrayOfParameterNamed("familyMembers")!=null)
			{
				this.clientFamilyMembersWritePlatformService.addClientFamilyMember(clientForUpdate, command);
			}
...
//            if(!checkCibil(clientId))
//            {
//            	throw new CreditScoreUnsatisFactoryException(clientId);
//            }
            final Locale locale = command.extractLocale();
            final DateTimeFormatter fmt = DateTimeFormatter.ofPattern(command.dateFormat()).withLocale(locale);
...
//				if(ClientScore>Configuredscore.getValue() || ClientScore<100l)
//				{
//					return true;
//				}
//				break;
