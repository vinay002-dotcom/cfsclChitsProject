...
public class FileUploadValidator {
    ...
    public boolean fileIsNull(InputStream inputStream, FormDataContentDisposition fileDetails, FormDataBodyPart bodyPart) {
        DataValidatorBuilder validator = new DataValidatorBuilder().resource("fileUpload").reset().parameter("InputStream").value(inputStream).notNull().reset()
        .parameter("FormDataContentDisposition").value(fileDetails).notNull().reset().parameter("FormDataBodyPart").value(bodyPart)
        .notNull();

    //////System.out.println(""+fileDetails + validator.getDataValidationErrors().isEmpty()); 

        if (validator.getDataValidationErrors().isEmpty()) 
            return true;
        else
            return false;
    }
